...
if(isset($_GET['msv'])) {
    ...
    $sql = "SELECT tbl_diem.msv,tbl_sinhvien.ten,tbl_hocphan.tenhocphan,tbl_diem.diemX,tbl_diem.diemY,tbl_diem.diemZ,tbl_diem.dotdk_id ,CASE WHEN tbl_diem.diemZ>=9 THEN 'A' WHEN tbl_diem.diemZ>=7 
THEN 'B'WHEN tbl_diem.diemZ>=5.5 THEN 'C' WHEN tbl_diem.diemZ>=4 THEN 'D' ELSE 'F'END as diemchu ,CASE WHEN tbl_diem.diemZ>=9 THEN '4' WHEN 
tbl_diem.diemZ>=7 THEN '3' WHEN tbl_diem.diemZ>=5.5 THEN '2' WHEN tbl_diem.diemZ>=4 THEN '1' ELSE '0' END as diemso,tbl_diem.ghichu FROM tbl_diem,tbl_sinhvien,tbl_hocphan WHERE tbl_diem.msv=tbl_sinhvien.msv
 AND tbl_diem.mahocphan=tbl_hocphan.mahocphan AND tbl_sinhvien.msv='$msv'";
    $arr = array();
...
THEN 'B'WHEN tbl_diem.diemZ>=5.5 THEN 'C' WHEN tbl_diem.diemZ>=4 THEN 'D' ELSE 'F'END as diemchu ,CASE WHEN tbl_diem.diemZ>=9 THEN '4' WHEN 
tbl_diem.diemZ>=7 THEN '3' WHEN tbl_diem.diemZ>=5.5 THEN '2' WHEN tbl_diem.diemZ>=4 THEN '1' ELSE '0' END as diemso,tbl_diem.ghichu FROM tbl_diem,tbl_sinhvien,tbl_hocphan WHERE tbl_diem.msv=tbl_sinhvien.msv
 AND tbl_diem.mahocphan=tbl_hocphan.mahocphan AND tbl_sinhvien.msv='$msv' GROUP BY dotdk_id";
...
<h2><input style="text-align: center" type="button" class="btn btn-success" value="Download Bảng Điểm" onclick="dowloaddiem()"></h2>
<script>
    function dowloaddiem() {


            var msv='<?php echo $msv;?>';
        window.location="./dowloaddiem.php?msv="+msv;
    }
</script>
...
foreach ($arr_dk as $item)
{
    $dotdk_id=$item["dotdk_id"];

    $sql="SELECT * FROM tbl_dotdk WHERE dotdk_id='$dotdk_id'";
    $result=$conn->query($sql);
    $ten_dotdk=$result->fetch_assoc();
    ?>
    <a><h1 style="text-align: center"><?php echo  $ten_dotdk["name"];?></h1></a>
...
<?php
    foreach ($arr as $diem)
    {
        if($item["dotdk_id"]==$diem["dotdk_id"])
        {
            ?>
