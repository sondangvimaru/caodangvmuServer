<?php

namespace common\models\search;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use common\models\TblBaidang;

/**
 * TblBaidangSearch represents the model behind the search form of `common\models\TblBaidang`.
 */
class TblBaidangSearch extends TblBaidang
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['baidang_id', 'danhmuc_id'], 'integer'],
            [['tieude', 'anh_dai_dien', 'noidung', 'thoigiandang'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = TblBaidang::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'baidang_id' => $this->baidang_id,
            'thoigiandang' => $this->thoigiandang,
            'danhmuc_id' => $this->danhmuc_id,
        ]);

        $query->andFilterWhere(['like', 'tieude', $this->tieude])
            ->andFilterWhere(['like', 'anh_dai_dien', $this->anh_dai_dien])
            ->andFilterWhere(['like', 'noidung', $this->noidung]);

        return $dataProvider;
    }
}
